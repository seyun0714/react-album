...
function Card({ prop, handleData }: Props) {
  const removeBookmark = (prop: CardDTO) => {
    const res = JSON.parse(localStorage.getItem("bookmark")).filter(
      (item: CardDTO) => item.id !== prop.id
    );
  ...
  return (
    <div className={styles.card}>
      <div
        className={styles.card__removeBox}
        onClick={() => removeBookmark(prop)}
      >
